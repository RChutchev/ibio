...
class KeyGeneratorService
{
    ...
    public function generate(string $value): string
    {
        ...
        if (
            ...
            || strlen($key) < config('urls.hash_length')
        ) {
    ...
    public function generateSimpleString(string $value): string
    {
        return Str::of($value)
            ...
            ->substr(config('urls.hash_length') * -1)
            ->lower();
    }
    ...
    public function generateRandomString(): string
    {
        ...
        do {
            $urlKey = $generator->generateString(config('urls.hash_length'), self::HASH_CHAR);
    ...
    public function assertStringCanBeUsedAsKey(string $value): bool
    {
        ...
        $alreadyInUse = Urls::whereKeyword($value)->exists();
        $isReservedKeyword = in_array($value, config('urls.reserved_keyword'));
    ...
    */
    ...
    public function possibleOutput(): int
    {
        ...
        $strLen= config('urls.hash_length');
    ...
    {
        $hashLength = (int) config('urls.hash_length');
